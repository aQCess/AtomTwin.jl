...
let B_MHz      = g_eff * µB/h * norm(B_vec) * 1e-6
    ...
    T_move_max = 10µs/µm * (ysep - ysep_gate)
    println("─────────────────────────────────────────────────────────────")
    ...
    @printf "  Max move duration      : %.1f us\n"             T_move_max/µs
...
function build_system(;
    ...
    tweezer = TweezerArray(
        ...
        w0        = 1µm,
        P_total   = 100mW,
        row_freqs = [(ysep_gate/2 - y_start)/µm * MHz, ysep_gate/(2µm) * MHz],
    ...
    d = y_start - ysep_gate
    t_move = iszero(d) ? 0.0 : 10 * (ysep - ysep_gate) * (d / (ysep - ysep_gate))^(1/3)
