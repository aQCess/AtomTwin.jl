...
const n_shots      = 100    # reduce for testing
...
y_start = ysep  # change to ysep for full circuit with shuttle
(; sys, ryd, atoms, tweezer, T_move) = build_system(y_start = y_start)

@info "System build complete"
...
@sequence seq begin
    ...
    MoveRow(tweezer, 1,  (y_start - ysep_gate)/µm * MHz, T_move; dt = 10dt)  # shuttle into blockade
    CZ()                                                                    # entangling gate
    X(1:4)                                                                  # dynamical decoupling echo
    MoveRow(tweezer, 1, -(y_start - ysep_gate)/µm * MHz, T_move; dt = 10dt)  # return row 1
...
F_raws, P_evens, code_pops, state1, traj_x, traj_y = if isfile(results_file)
    ...
    d = JSON3.read(read(results_file, String), Dict{String,Any})
    (Float64.(d["F_raws"]), Float64.(d["P_evens"]), Float64.(d["code_pops"]),
     complex.(Float64.(d["state1_re"]), Float64.(d["state1_im"])),
     [Float64.(v) for v in d["traj_x"]],
     [Float64.(v) for v in d["traj_y"]])
else
    println("Running Bell state encoding (n_shots = $n_shots)...")
    print("  ")
    job = compile(sys, seq; initial_state = [qubit0, qubit0, qubit0, qubit0])
    out = play(job, sys;
               savefinalstate = true,
               shots          = n_shots,
               shot_callback  = (s, _) -> s % 1 == 0 && print("."))
    println()
    ...
    _state1 = out.final_states[1]
    _traj(name, dim) = let det = out.detectors[name]
        ndims(det) == 2 ? det[:, dim] : det[:, dim, 1]
    end
    _traj_x = [_traj("atom_$i", 1) for i in 1:length(atoms)]
    _traj_y = [_traj("atom_$i", 2) for i in 1:length(atoms)]
    ...
    open(results_file, "w") do io
        JSON3.write(io, Dict{String,Any}(
            ...
            "state1_re"  => real.(_state1),
            "state1_im"  => imag.(_state1),
            "traj_x"     => _traj_x,
            "traj_y"     => _traj_y,
        ))
    end
    println("Saved results to $results_file")
    (_F_raws, _P_evens, _code_pops, _state1, _traj_x, _traj_y)
end
